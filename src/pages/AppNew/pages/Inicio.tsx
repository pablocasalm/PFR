...
/**
 * Inicio — Dashboard principal del nuevo /appnew. Solo visual (datos mock).
...
const ContinueCard = ({ item }: { item: ContinueItem }) => (
  <Link to="/appnew/watch?v=1" className="flex overflow-hidden rounded-xl border border-white/10 bg-white/[0.02] transition hover:border-white/20">
...
const NuevoCard = ({ card }: { card: MediaCard }) => (
  <Link
    to={card.kind === "analisis" ? "/appnew/watch?v=1" : "/appnew/watch?c=1"}
...
const RankedCardItem = ({ card }: { card: RankedCard }) => (
  <Link to="/appnew/watch?c=1" className="group block w-full cursor-pointer">
    <div className="relative overflow-hidden rounded-xl border border-white/10">
      <div className="aspect-video w-full" style={thumbStyle(card.hue)} />
