...
/**
 * Header compartido del nuevo dashboard (/appnew).
 * Usa NavLink para resaltar la sección activa de forma real.
 */

const NAV_ITEMS = [
  { label: "Inicio", to: "/appnew/inicio" },
  { label: "Explorar", to: "/appnew/explorar" },
  { label: "Mi Lista", to: "/appnew/mi-lista" },
  { label: "Mi Juego", to: "/appnew/mi-juego" },
]
...
const Header = () => {
  ...
  const submitSearch = () => {
    const q = query.trim()
    if (q) navigate(`/appnew/search?q=${encodeURIComponent(q)}`)
  }
